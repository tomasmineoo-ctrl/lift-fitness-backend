...
router.get('/log', authorize('admin', 'reception'), async (req: Request, res: Response) => {
  ...
  let query = supabase.from('access_log').select('*', { count: 'exact' }).eq('gym_id', gymId).order('log_timestamp', { ascending: false });
...
router.post('/entry', authorize('admin', 'reception'), async (req: Request, res: Response) => {
  ...
  const { data: user } = await supabase.from('users').select('name, status, frozen, last_payment, plan').eq('id', user_id).eq('gym_id', gymId).single();
  ...
  if (allowed) {
    await supabase.from('points_log').insert({ user_id, user_name: user.name, action: 'asistencia', points: 10, detail: 'Check-in', gym_id: gymId });
  }
...
router.post('/verify', async (req: Request, res: Response) => {
  ...
  let userId = user_id;
  if (!userId && pin) {
    const { data } = await supabase.from('users').select('id').eq('pin', pin).eq('gym_id', gymId).single();
  ...
  const { data: user } = await supabase.from('users').select('id, name, status, frozen, last_payment, plan, med_clearance').eq('id', userId).eq('gym_id', gymId).single();
